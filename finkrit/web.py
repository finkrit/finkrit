#!/usr/bin/env python3
"""
The web launcher: the FastAPI server (real LLM, real market data) plus the
frontend, with your browser opened for you. Reached as ``finkrit`` or
``finkrit web``.

    finkrit                    build the web app once and serve it
    finkrit --dev              Vite hot reload, for working on the frontend
    finkrit --model openai:gpt-5
    finkrit --no-browser --port 8001
...
Two ways to serve the UI:
    ...
    --dev     Run the Vite dev server alongside the API so UI edits hot reload.
              Vite serves on 5173 and proxies /api to the FastAPI port. Both
              processes stop together on Ctrl C.

One env var regardless of provider: LLM_API_KEY (or pass --key). pydantic-ai's
providers each default to reading their OWN var (ANTHROPIC_API_KEY,
OPENAI_API_KEY, and so on). We override the provider construction step to inject
LLM_API_KEY as that provider's api_key, so switching --model does not mean
switching which env var you export.
"""
...
def resolve_model_or_warn(model_string: str):
    ...
    try:
        ...
        print("  finkrit --key sk-...          the same, on the command line")
        if native:
...
def ensure_web_deps() -> None:
    # Install the web app's node_modules on first use, so running on a fresh
    # clone does not fail before the build or dev server starts.
...
def main(argv: list[str] | None = None) -> None:
    ...
    if args.dev:
        if not WEB_DIR.is_dir():
            print("The --dev flag needs the web source (apps/finkritweb), which ships "
                  "only in a source checkout, not the installed package.")
            sys.exit(1)
        vite = start_vite_dev()
    elif not args.skip_build and WEB_DIR.is_dir():
        # Source checkout: build the UI. Installed from a wheel the built UI is
        # already bundled and served by finkritserver, so there is nothing to
        # build here.
        build_frontend()
