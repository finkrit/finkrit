# finkrit/packages/finkritq/tests/transform/test_returns.py
"""
Tests for finkritq.transform.returns.periodic_returns().
"""
...
class TestReturnLength:
    ...
    def test_log_length_is_n_minus_1(self):
        r = periodic_returns(MARKET_PRICES, ReturnCalculationMethod.LOG)
        assert len(r) == len(MARKET_PRICES) - 1

    def test_simple_length_is_n_minus_1(self):
        r = periodic_returns(MARKET_PRICES, ReturnCalculationMethod.SIMPLE)
        assert len(r) == len(MARKET_PRICES) - 1

    def test_two_prices_gives_one_return(self):
        r = periodic_returns(np.array([100.0, 110.0]))
        assert len(r) == 1

    def test_returns_1d_array(self):
        r = periodic_returns(MARKET_PRICES)
        assert r.ndim == 1
...
class TestLogReturnValues:
    ...
    def test_known_doubling(self):
        # ln(200/100) = ln(2)
        r = periodic_returns(np.array([100.0, 200.0]), ReturnCalculationMethod.LOG)
        assert r[0] == pytest.approx(np.log(2))

    def test_known_halving(self):
        r = periodic_returns(np.array([100.0, 50.0]), ReturnCalculationMethod.LOG)
        assert r[0] == pytest.approx(-np.log(2))
    ...
    def test_flat_prices_zero_returns(self):
        r = periodic_returns(FLAT_PRICES, ReturnCalculationMethod.LOG)
...
class TestSimpleReturnValues:
    ...
    def test_ten_percent_gain(self):
        r = periodic_returns(np.array([100.0, 110.0]), ReturnCalculationMethod.SIMPLE)
        assert r[0] == pytest.approx(0.10)

    def test_twenty_percent_loss(self):
        r = periodic_returns(np.array([100.0, 80.0]), ReturnCalculationMethod.SIMPLE)
    ...
    def test_flat_prices_zero_returns(self):
        r = periodic_returns(FLAT_PRICES, ReturnCalculationMethod.SIMPLE)
        np.testing.assert_allclose(r, 0.0, atol=1e-12)
...
class TestDefaultMethod:
    ...
    def test_default_is_log(self):
        default = periodic_returns(MARKET_PRICES)
        explicit = periodic_returns(MARKET_PRICES, ReturnCalculationMethod.LOG)
...
class TestErrorHandling:
    ...
    def test_unsupported_method_raises(self):
        with pytest.raises((ValueError, AttributeError)):
            periodic_returns(MARKET_PRICES, method="bad_method")  # type: ignore
...
class TestReturnProperties:
    ...
    def test_rising_prices_all_positive_simple(self):
        r = periodic_returns(RISING_PRICES, ReturnCalculationMethod.SIMPLE)
        assert np.all(r > 0)

    def test_rising_prices_all_positive_log(self):
        r = periodic_returns(RISING_PRICES, ReturnCalculationMethod.LOG)
        assert np.all(r > 0)
    ...
    def test_log_approx_simple_for_small_returns(self):
        ...
        log_r    = periodic_returns(prices, ReturnCalculationMethod.LOG)
        simple_r = periodic_returns(prices, ReturnCalculationMethod.SIMPLE)
