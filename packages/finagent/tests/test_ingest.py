...
"""
Tests for the model fallback: CSV -> ParsedPortfolio via an LLM.

The deterministic mapper that runs first lives in finkritcore, and so do its
tests. Every CSV here is deliberately missing a column so it reaches the model
at all.

Original note on the mechanism: Structured output_type in
